...
maps.v["<C-c>"] = { '"*y', noremap = true, desc = "Copy to system clipboard" }
maps.v["<leader>p"] = { [["_dP]], desc = "Paste and retain paste in clipboard" }

-- vim.keymap.set("n", "<leader>s", [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]])
maps.n["<leader>s"] =
{ [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]], desc = "Search and replace highlighted" }

-- Keep cusor in index position on J
maps.n["J"] = { "mzJ`z", desc = "Keep cusor in index position on J" }

-- Keep cusor in middle of buffer
maps.n["<C-d>"] = { "<C-d>zz", desc = "⬇ middle of buffer and keep cusor in middle of buffer" }
maps.n["<C-u>"] = { "<C-u>zz", desc = "⬆ middle of buffer and keep cusor in middle of buffer" }
maps.n["n"] = { "nzzzv", desc = "next search and keep cusor in middle of buffer" }
maps.n["N"] = { "Nzzzv", desc = "previous search and keep cusor in middle of buffer" }

-- Move highlighted up or down
maps.v["J"] = { ":m '>+1<CR>gv=gv", desc = "Move highlighted up" }
maps.v["K"] = { ":m '<-2<CR>gv=gv", desc = "Move highlighted down" }
...
if is_available "smart-splits.nvim" then
  ...
  maps.n["<leader>h"] = { "<C-w>h", desc = "Move to left split" }
  maps.n["<leader>j"] = { "<C-w>j", desc = "Move to below split" }
  maps.n["<leader>k"] = { "<C-w>k", desc = "Move to above split" }
  maps.n["<leader>l"] = { "<C-w>l", desc = "Move to right split" }
